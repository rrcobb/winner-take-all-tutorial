// This code represents our Winner Takes All game after completing the tutorial
      ...
      <div className={`player ${this.props.odd ? 'right' : 'left'}`}>
        <div className={'cards'}>
          ...
          <div className={'table-spot'}>
            {this.props.card && <Card value={this.props.card.value} suit={this.props.card.suit} />}
          </div>
        </div>
...
const reducer = (state = defaultState, action) => {
  switch (action.type) {
    ...
    case 'Resolve': {
      const sorted = Object.values(state).sort((a, b) => a.card.value < b.card.value);
      ...
      <Provider store={store}>
        <div className="App">
          <div className="App-header">
            ...
            <small>
              <a href="https://github.com/rrcobb/winner-take-all-tutorial">(♥ Fin)</a>
            </small>
          </div>
          <div className="gameboard">
            {players.map((name, index) => <Player name={name} odd={index % 2 !== 0} />)}
          </div>
          <Controls />
        </div>
      </Provider>
